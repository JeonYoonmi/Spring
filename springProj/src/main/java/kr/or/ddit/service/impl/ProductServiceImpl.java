...
// 프링이에게 이 클래스는 서비스 클래스이라는 것을 알려주면, 자바 빈으로 등록하여 관리 => 자바빈으로 만든다는 것은 메모리에 올려서 객체로 만들겠다는 말.
@Service
public class ProductServiceImpl implements ProductService {
	// 주입 주체? 개발자(x), 프링이(O) => IoC(Inversion of Control : 제어의 역전)
	// DI(Dependency Injection : 의존성 주입)
	// ProductServiceImpl 클래스에서 productDao객체를 주입하여 사용
	@Autowired
	ProductDao productDao;
	
	// Override : 슈퍼타입(부모)의 메소드를 재정의하여 상세화
	// Overloading : 동일 클래스에서 같은 이름의 메소드를 여러번 사용(파라미터 개수, 파라미터 타입)
	// 상품 등록
	@Override
