						...
						<div id="dataTable_filter" class="dataTables_filter">
							<label>검색 
								<select name="cond" aria-controls="dataTable"
								  ...
								  <option value="writerName" <c:if test="${ param.cond == 'writerName' }">selected</c:if>>작성자</option>
					...
					<div class="col-sm-12">
						<table class="table table-bordered dataTable" id="dataTable"
							...
							<tbody>
								...
								<!-- 
									- data
										ㄴ 페이징 처리 전 : List<ArticleVO> list
										ㄴ 페이징 처리 후 : ArticlePage 객체 
								-->
				...
				<div class="row">
					<div class="col-sm-12 col-md-4">
						<div class="dataTables_info" id="dataTable_info" role="status"
							...
							<!-- 
								- scope(공유 영역) : page(기본), request, session, application
								- 종료 행 : currentPage * size 
								- 시작 행 : 종료행 - (size - 1)
								- 전체 행 수 : total
							-->
					...
					<div class="col-sm-12 col-md-6">
						<div class="dataTables_paginate paging_simple_numbers"
							...
							<!-- class="... disabled" => 비활성 -->
							<ul class="pagination">
								<li class="paginate_button page-item previous <c:if test='${ data.startPage lt 6 }'>disabled</c:if>"
									id="dataTable_previous"><a href="/article/list?show=${ map.show }&cond=${ map.cond }&keyword=${ map.keyword }&currentPage=${ data.startPage - 5 }"
									aria-controls="dataTable" data-dt-idx="0" tabindex="0"
									class="page-link">Previous</a></li>
								<!-- class=" ... active" => 현재 페이지. 파란색을 띔 -->
								<c:forEach var="pNo" begin="${ data.startPage }" end="${ data.endPage }">
								<li class="paginate_button page-item <c:if test="${ data.currentPage == pNo }">active</c:if>">
								<!-- 
								...
								<!-- 
									- EL 태그 정리
										...
										- >=	: ge(greater equal)
								 -->
								<li class="paginate_button page-item next <c:if test="${ data.endPage ge data.totalPages }">disabled</c:if>" id="dataTable_next"><a
...
<!-- 								<a href="/article/write" class="btn btn-light btn-icon-split btn-sm" style="position: absolute; left: 80%;"> -->
...
<!-- 			                    </a> -->
						...
						<div class="col-sm-12 col-md-2">
							<a href="/article/write" class="btn btn-light btn-icon-split btn-sm">
                              <span class="icon icon text-gray-50">
