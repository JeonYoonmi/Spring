			...
			<!-- 
				...
				1) 인증정보
				 ...
				 - principal : 로그인 한 사용자 정보(UserDetails 인터페이스를 구현한 클래스의 객체)
				   1(username, password) : N(athorities)
				2) 권한 정보
				 - hasRole(role명) : 해당 role이 있으면 true
				 - hasAnyrole(role명1, role명2) : 여러 role 중 하나라도 해당하는가?
			 -->
<%-- 			<c:if test="${ sessionScope.sessionId.id == null }"> --%>
<!-- 				<a class="mr-2 d-none d-lg-inline text-gray-600 small" href="/member/loginMember" style="margin-top: 15%;">로그인</a> -->
<%-- 			</c:if> --%>
			...
			<!-- 로그인이 된 경우 시작 -->
			<!-- 로그인 성공 -> session.setAttribute("sessionId", pmemberVO); -->
			<!-- ${ sessioScope.sessionId } => PmemberVO pmemberVO 객체 -->
<%-- 			<c:if test="${ sessionScope.sessionId.id != null }"> --%>
			<!-- 
				isAuthenticated() : 인증된 사용자면 true(로그인 했을 경우)
				isAnonymous() : 익명의 사용자의 경우 treu(로그인 안했을 경우)
			 -->
	        ...
	        <!-- Nav Item - User Information -->
		        <li class="nav-item dropdown no-arrow">
		            <a class="nav-link dropdown-toggle" href="#" id="userDropdown" role="button"
		                data-toggle="dropdown" aria-haspopup="true" aria-expanded="false">
		                <!-- (달러){ sessionScope.sessionId.name }님 -->
						<!-- 
							principal 객체 : 로그인된 사용자의 사용자 정보를 담고 있는 객체
						 -->
		                <span class="mr-2 d-none d-lg-inline text-gray-600 small">
		                	<sec:authentication property="principal.memVO.userName"/>(<sec:authentication property="principal.username"/>)님
		                </span>
